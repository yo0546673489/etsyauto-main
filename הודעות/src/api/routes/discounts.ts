...
export async function discountRoutes(
  ...
  // ===== רוטציית הנחות (תזמון אוטומטי) =====

  // שמירת לוח זמנים
  fastify.post('/api/discounts/schedule', async (request, reply) => {
    const body = request.body as {
      storeId: number;
      scheduleName: string;
      rotationConfig: Record<string, { percent: number; saleName: string }>;
      targetScope: 'whole_shop' | 'specific_listings';
      listingIds?: string[];
      targetCountry: string;
      termsText?: string;
    ...
    // rotationConfig example:
    // { "0": { "percent": 25, "saleName": "MondaySale" },
    //   "1": { "percent": 15, "saleName": "TuesdaySale" }, ... }
    ...
    await pool.query(
      `INSERT INTO discount_schedules
       ...
       RETURNING id`,
      [
        body.storeId, body.scheduleName, JSON.stringify(body.rotationConfig),
        body.targetScope, body.listingIds || [], body.targetCountry, body.termsText || null,
      ]
    );
  ...
  // שליפת לוחות זמנים
  fastify.get('/api/discounts/schedules/:storeId', async (request) => {
    const { storeId } = request.params as { storeId: string };
    const result = await pool.query(
