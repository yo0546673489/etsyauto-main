...
export class DiscountTaskExecutor {
  ...
  private async checkAndEnqueue(): Promise<void> {
    logger.debug('Checking for pending discount tasks...');
  ...
  private async processTask(task: DiscountTask): Promise<void> {
    ...
    // Check that we actually updated (race condition protection)
    ...
    if (task.action === 'apply_discount') {
      ...
      // Calculate dates
      // Use tomorrow as start date to avoid "must not be in the past" errors on Etsy
      // (Etsy UK shop may be in a different timezone, so today could already be "past")
      ...
      await this.queue.add('execute', {
        ...
        platformTaskId: task.id,  // for updating platform DB
        saleConfig: {
          saleName,
  ...
  /**
   * Called by executeDiscount worker to update status in platform DB
   */
  async markTaskCompleted(platformTaskId: number, success: boolean, errorMessage?: string): Promise<void> {
    if (success) {
      await this.platformPool.query(
