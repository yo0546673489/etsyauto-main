/**
 * Scrape all conversations from any store by store_number
 * Usage: node cdp-scrape-store.js <store_number>
 */
...
const STORE_NUMBER = parseInt(process.argv[2] || '1');
...
async function humanClick(ws, x, y) {
  for (let i=0; i<rand(3,5); i++) {
    await ws.call('Input.dispatchMouseEvent', { type:'mouseMoved', x: x+rand(-15,15), y: y+rand(-8,8), button:'none' });
    await delay(rand(25,60));
  }
...
function getActiveProfile(profileId) {
  return new Promise((resolve, reject) => {
    http.get(`http://local.adspower.net:50325/api/v1/browser/active?user_id=${profileId}`, res => {
      let d=''; res.on('data',c=>d+=c); res.on('end',()=>{const j=JSON.parse(d); j.code===0?resolve(j.data):reject(new Error(j.msg));});
    }).on('error', reject);
  });
}
...
function openProfile(profileId) {
  return new Promise((resolve, reject) => {
    http.get(`http://local.adspower.net:50325/api/v1/browser/start?user_id=${profileId}`, res => {
      let d=''; res.on('data',c=>d+=c); res.on('end',()=>{const j=JSON.parse(d); j.code===0?resolve(j.data):reject(new Error(j.msg));});
    }).on('error', reject);
  });
}
...
const SCRAPE_MSGS = `(function() {
  ...
  bubbles.forEach(el => {
    ...
    // Combine unique image/link URLs
...
async function main() {
  ...
  console.log(`\n=== Scraping Store ${STORE_NUMBER}: ${store.store_name} (${store.adspower_profile_id}) ===`);

  // Open profile if needed
  let apData;
  try {
    apData = await getActiveProfile(store.adspower_profile_id);
    console.log('Profile already active');
  } catch(e) {
    console.log('Opening profile...');
    apData = await openProfile(store.adspower_profile_id);
    await delay(4000);
  }
  ...
  // Find or create Etsy page
  let etsyTarget = targets.find(t => t.type==='page' && t.url.includes('etsy.com'));
  if (!etsyTarget) {
    // Use first page and navigate
    etsyTarget = targets.find(t => t.type==='page');
    console.log('No Etsy page found, using first page:', etsyTarget?.url);
  }
  if (!etsyTarget) { console.error('No page target found'); process.exit(1); }

  console.log('Page:', etsyTarget.url);
  ...
  // Inject red cursor dot so user can see mouse movements
  await ws.call('Runtime.evaluate', {
    expression: `(function() {
      ...
      dot.style.cssText = 'position:fixed;width:18px;height:18px;background:red;border-radius:50%;pointer-events:none;z-index:2147483647;transform:translate(-50%,-50%);box-shadow:0 0 8px 3px rgba(255,0,0,0.6);transition:left 0.05s,top 0.05s;left:50%;top:50%';
      document.body.appendChild(dot);
      document.addEventListener('mousemove', function(e) {
        dot.style.left = e.clientX + 'px';
        dot.style.top = e.clientY + 'px';
      });
      return 'red dot injected';
    })()`,
    returnByValue: true
  }).then(r => console.log('Cursor:', r.result?.value)).catch(() => {});
  ...
  while (scrollRound < 5) {
    ...
    console.log(`\nRound ${scrollRound+1}: ${convRows.length} conversation rows`);
    ...
    for (let i=0; i<convRows.length; i++) {
      ...
      try {
        ...
        if (!convUrl.match(/\/messages\/\d+/)) {
          console.log(`    No conv URL: ${convUrl}`);
          continue;
        }
        ...
        for (const msg of scraped.messages) {
          ...
          // Fetch Etsy product card data for any listing URLs
          let cardData = {};
          const listingUrls = imgArr.filter(u => u.includes('etsy.com/listing/'));
          if (listingUrls.length > 0) {
            try {
              const listingUrl = listingUrls[0];
              console.log(`    Fetching card data: ${listingUrl.substring(0,60)}`);
              const currentUrl2 = (await ws.call('Runtime.evaluate', { expression: 'window.location.href', returnByValue: true })).result?.value;
              ...
              const CARD_EXTRACT = `(function() {
                ...
                // Price selectors for Etsy listing pages
                const saleSel = [
                  '[data-testid="price-only"] .currency-value',
                  '[class*="price"] .wt-text-title-larger',
                  '.wt-text-title-larger.wt-mr-xs-2'
                ];
                let salePrice = '';
                for (const sel of saleSel) {
                  const el = document.querySelector(sel);
                  if (el && el.textContent.trim()) { salePrice = el.textContent.trim().replace(/[^\\d.]/g,''); break; }
                }
                // fallback: look for currency symbol spans
                if (!salePrice) {
                  const cur = document.querySelector('.currency-symbol');
                  const val = document.querySelector('.currency-value');
                  if (val) salePrice = val.textContent.trim();
                }
                const origSel = [
                  '[data-testid="StrikeThroughPrice"] .currency-value',
                  '.wt-text-strikethrough .currency-value',
                  '[class*="original"] .currency-value'
                ];
                let origPrice = '';
                for (const sel of origSel) {
                  const el = document.querySelector(sel);
                  if (el && el.textContent.trim()) { origPrice = el.textContent.trim().replace(/[^\\d.]/g,''); break; }
                }
              ...
              if (parsed.image || parsed.title) {
                ...
                console.log(`    Card: "${(parsed.title||'').substring(0,40)}" img=${!!parsed.image} price=${parsed.salePrice}`);
              }
              // Navigate back to messages list
              await ws.call('Page.navigate', { url: 'https://www.etsy.com/messages/all' });
              await delay(rand(2000,3000));
            } catch(cardErr) {
              console.log(`    Card fetch error: ${cardErr.message}`);
        ...
        await ws.call('Page.navigate', { url: 'https://www.etsy.com/messages/all' });
        await delay(rand(2500,4000));
        // Re-inject red dot after navigation
        await ws.call('Runtime.evaluate', { expression: "(function(){const d=document.createElement('div');d.id='__red_cursor__';d.style.cssText='position:fixed;width:18px;height:18px;background:red;border-radius:50%;pointer-events:none;z-index:2147483647;transform:translate(-50%,-50%);box-shadow:0 0 8px 3px rgba(255,0,0,0.6);left:50%;top:50%';document.body.appendChild(d);document.addEventListener('mousemove',function(e){d.style.left=e.clientX+'px';d.style.top=e.clientY+'px';});return 'ok';})()", returnByValue: true }).catch(()=>{});
    ...
    if (!anyNew) {
      console.log('No new conversations found, trying to scroll...');
      await ws.call('Runtime.evaluate', { expression: "(function(){ const l=document.querySelector('[class*=\"inbox\"]'); if(l)l.scrollTop+=400; else window.scrollBy(0,400); return 'ok'; })()", returnByValue: true });
      await delay(2000);
    }
    scrollRound++;
  }

  const total = await pool.query('SELECT COUNT(*) as cnt FROM conversations WHERE store_id=$1', [store.id]);
  const msgs = await pool.query('SELECT COUNT(*) as cnt FROM messages m JOIN conversations c ON m.conversation_id=c.id WHERE c.store_id=$1', [store.id]);
  console.log(`\n=== Done: ${saved} new, ${failed} failed, ${skipped} skipped ===`);
  console.log(`DB: ${total.rows[0].cnt} conversations, ${msgs.rows[0].cnt} messages`);
